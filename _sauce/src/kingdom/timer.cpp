...
bool(*dk::TIMER::isDown)() = ResolveFunctionFromCall<bool(*)()>("\x40\x55\x53\x56\x57\x41\x56\x48\x8B\xEC\x48\x83\xEC\x60\x48\x8B\xD9\xE8", "xxxxxxxxxxxxxxxxxx", 0x27F);
uint32_t(*dk::TIMER::getTime)() = ResolveFunctionFromCall<uint32_t(*)()>("\x40\x55\x53\x56\x57\x41\x56\x48\x8B\xEC\x48\x83\xEC\x60\x48\x8B\xD9\xE8", "xxxxxxxxxxxxxxxxxx", 0x10A);

void dk::TIMER::update(char* timer)
{
	...
	*reinterpret_cast<int*>(timer + 0x0028) = _applyAspect + _fetchActiveX;
	...
	else
	{
		...
		if (_timeDisplay != _timeCurrent)
		{
			...
			sprintf(_arrayMinuteNum, "%d", _currMinute);
			sprintf(_arraySecondNum, "%d", _currSecond);
			sprintf(_arraySaliseNum, "%d", static_cast<int>(_currSalise));
...
dk::TIMER::staticInitializer initialize;